#!/usr/bin/env node
/* SOCIAL UP! 教材データ整形。実行: node scripts/format-content.mjs（Node.js 18 以上、追加パッケージ不要）
...
/* ファイルごとに「どの深さまで改行で展開するか」を決める。
   index.json / geo,his,civ: 2（問題1件を1行）
   order.json: 4（セットを複数行にし、できごと1件を1行にする） */
function depthFor(f){ return f === 'order.json' ? 4 : 2; }
